...
def tile_effect(players, current_p, board):
    ...
    if 'none' in board[position]:
        ...
        if ask_for_choice(('step', board[position][1]), 'health'):
            ...
            visual_a.create_hide_roll()
            visual_a.activated_effect('Step modified, {0}'.format(board[position][1]))
            visual_a.delete_hide_roll()
        else:
            (model.step(players, current_p, board[position][1], True))
            visual_a.create_hide_roll()
            visual_a.activated_effect('Health modified, {0}'.format(board[position][1]))
            visual_a.delete_hide_roll()
        ...
        if ask_for_choice(('turn_mod', board[position][1]), 'health'):
            ...
            visual_a.create_hide_roll()
            visual_a.activated_effect('Turn modified, {0}'.format(board[position][1]))
            visual_a.delete_hide_roll()
        else:
...
def turn_handler():
    global current_p
    turn(model.players, current_p, model.board)
    if check_win(visual_b.input_data('board length'), model.players, current_p):
        display_winner(current_p)
    current_p = current_player_generator(current_p, visual_b.input_data('number of players'))


# A single turn structure
def turn(players, current_p, board):
    visual_a.print_turn(current_p)
    visual_a.print_health(players[current_p])
    print(players)
    ...
    if players[current_p][0] + steps > len(board) - 1:
        ...
        visual_a.print_move(players, current_p)
        time.sleep(.10)
...
def ask_for_info():
    ...
    # dice_v = 6
    # level_v = 'Medium'
    length_v = visual_b.input_data('board length')
    n_players_v = visual_b.input_data('number of players')
    dice_v = visual_b.input_data('dice')
    level_v = visual_b.input_data('difficulty')
...
def setup(board, players):
    print('k')
    visual_a.print_board(board)
    print('kk')
    visual_a.sidebar()
    print('kkkk')
    visual_a.make_players(players, 0)
    return
...
def ask_for_choice(option0, option1):
    ...
    a, b = visual_a.choice(option0, option1)
    print('a', a)
    print('b', b)
    return a == 0
...
#     import visual_a
#
#     visual_a.move_player(player, diceval)
#     return
...
def display_winner(current_p):
    visual_a.win_effect(current_p)
    time.sleep(3)
    quit()
    #visual_a.slide_win(('Player', current_p, 'won the game!'), current_p)
